...
class VisitSensor:
    ...
    and returns the number of visitors that passes through
    ...
    def get_visit_count(self, business_date: date) -> int:
        ...
        if proba_malfunction < self.perc_malfunction:
            visit = np.floor(visit * 0.2) # make it so bad we can detect
